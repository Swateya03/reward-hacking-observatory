...
**Raw API access instead of LangChain:** The agent needs access to Claude's log probabilities (to measure confidence), the full trajectory (to detect hacking patterns), and precise control over tool use and stop reasons. LangChain abstracts all of this. Instead, the agent uses the Anthropic Python SDK directly, collecting tool_use and stop_reason from the raw API response. This is the only way to instrument the system deeply enough to detect subtle reward hacking.
